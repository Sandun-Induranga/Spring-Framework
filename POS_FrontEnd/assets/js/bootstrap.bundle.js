...
(function (global, factory) {
  ...
  function normalizeDataKey(key) {
    return key.replace(/[servlets.A-Z]/g, chr => `-${chr.toLowerCase()}`);
  }
  ...
  const enableDismissTrigger = (component, method = 'hide') => {
    ...
    EventHandler.on(document, clickEvent, `[data-bs-dismiss="${name}"]`, function (event) {
      if (['servlets.A', 'AREA'].includes(this.tagName)) {
  ...
  EventHandler.on(document, EVENT_CLICK_DATA_API$4, SELECTOR_DATA_TOGGLE$4, function (event) {
    ...
    if (event.target.tagName === 'servlets.A' || event.delegateTarget && event.delegateTarget.tagName === 'servlets.A') {
      event.preventDefault();
    }
  ...
  */
  ...
  } // servlets.A "clipping parent" is an overflowable container with the characteristic of
  ...
  /**
   * servlets.A pattern that recognizes a commonly useful subset of URLs that are safe.
   *
  ...
  /**
   * servlets.A pattern that matches safe data URLs. Only matches image, video and audio types.
   *
   * Shout-out to Angular https://github.com/angular/angular/blob/12.2.x/packages/core/src/sanitization/url_sanitizer.ts
   */
