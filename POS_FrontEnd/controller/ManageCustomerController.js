...
const baseUrl = "http://localhost:8080/spring_boot/";
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            for (let customer of res.data) {
                $("#body").append(`<tr><td>` + customer.id + `</td><td>` + customer.name + `</td><td>` + customer.address + `</td><td>` + customer.salary + `</td><td><button type="button" class="border border-0 customer-edits bg-transparent"><i class="bi bi-pencil-fill text-success" data-bs-toggle="modal" data-bs-target="#staticBackdrop"></i></button><button class="border border-0 customer-deletes bg-transparent"><i class="bi bi-trash text-danger"></i></button></td></tr>`);
            }
